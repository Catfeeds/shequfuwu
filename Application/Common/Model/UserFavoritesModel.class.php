...
//崔
    ...
    public function getFavoritesList($condition = array(), $relation = false, $order = "id desc", $p = 0, $num = 0, $limit = 0)
    {
    ...
    public function getFavoritesListCount($condition = array())
    {
        $count = $this->where($condition)->count();
        return $count;
    }

    public function addAllFavorites($data)
    {
        $this->addAll($data);
    }

    public function delFavorites($product_ids, $userId)
    {
        $this->where(array("product_id" => array("in", $product_ids), "user_id" => $userId))->delete();
    }
