...
template('productList-container', function ($data, $filename
                                            /**/) {
    'use strict';
    var $utils = this, $helpers = $utils.$helpers, $each = $utils.$each, menu = $data.menu, value = $data.value, i = $data.i, $escape = $utils.$escape, product = $data.product, imageUrl = $data.imageUrl, uploadsUrl = $data.uploadsUrl, $out = '';
    $out += '<div class="header-bar"> <div id="header-searchInput"> <div class="input-group"> <div class="input-text"> <i class="iconfont">&#xe601;</i> <div class="ui-suggestion-mask"> <input class="icon-input" name="searchtxt" type="search" placeholder="输入搜索商品"> <div class="widget-common-eraser widget-common-eraser-hidden"></div> </div> </div> <input type="button"class="search-btn" onclick="openSearch()" value="搜索"> </div> </div> </div> <ul class="shop-menu"> ';
